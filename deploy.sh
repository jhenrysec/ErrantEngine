...
if [ "$INSTALL_SERVICE" = "yes" ]; then
  if have_systemd; then
    need_root
    # `python3 -m http.server --directory` was added in Python 3.7. Older interpreters
    # (e.g. Debian 9 / Python 3.5) exit 2/INVALIDARGUMENT on it, so omit the flag there
    # and let WorkingDirectory= set the serve root.
    if "$PYTHON_BIN" -c 'import sys;exit(0 if sys.version_info[:2]>=(3,7) else 1)'; then
      DIR_FLAG=" --directory ${WEB_ROOT}"
    else
      DIR_FLAG=""
      c_info "python3 < 3.7 detected — omitting --directory (serve root = WorkingDirectory)"
    fi
    cat > "$UNIT_PATH" <<EOF
[Unit]
...
ExecStart=${PYTHON_BIN} -m http.server ${PORT} --bind ${BIND}${DIR_FLAG}
Restart=on-failure
RestartSec=3
User=${SVC_USER}
Group=${SVC_USER}
...
if [ "$INSTALL_SERVICE" = "yes" ]; then
  ...
  c_info "Run now  : cd ${WEB_ROOT} && ${PYTHON_BIN} -m http.server ${PORT} --bind ${BIND}"
fi
